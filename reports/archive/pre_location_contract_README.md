...
### Focused food-rank review

[Review results](reports/food_rank_review.json) record the cassava stem-cutting correction, a separate inland-varzea rotation candidate and Kansai's rice-only chart interpretation. `configs/rotations.json` keeps temporal rotation assumptions independent of management effectiveness. Reproduce comparisons with `uv run python scripts/review_food_ranking.py` after rebuilding. The previous maps are preserved in `artifacts/experiments/pre_food_rank_review`.

### Water-only report

Run uv run ha1300 water after uv run python scripts/acquire_water.py.
Open artifacts/water/index.html for three synchronized reference-water maps.
See [water methodology](reports/water_method.md) for scope, sources and limitations;
validate with uv run python scripts/validate_water.py.
