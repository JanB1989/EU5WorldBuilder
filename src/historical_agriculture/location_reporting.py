...
METRICS=[('starting_capacity','Starting capacity','people'),('maximum_capacity','Maximum capacity','people'),('inert_capacity','Natural capacity','people'),('starting_improvement_capacity','Starting improvement contribution','people'),('maximum_improvement_capacity','Maximum improvement contribution','people'),('remaining_capacity','Remaining improvement contribution','people'),('starting_fill','Starting population / capacity','ratio'),('inferred_area_share','Inferred input share','share')]
...
def report(out,d,raw,cfg,audit,fingerprint):
    ...
    from .capacity_targets import write as write_targets
    target_checks={'physical':write_targets(out,d,'physical'),'equal_area':write_targets(out,equal,'equal_area')}
    ...
    from .fill_report import report as fill_report
    fill_report(out,equal,cfg,fingerprint)
    ...
    notice+=f"<p class='tag'>Game targets are people: <a href='capacity_targets_equal_area.csv'>natural, starting and maximum capacity</a> with a <a href='improvement_ledger_equal_area.csv'>disjoint typed improvement ledger</a>. The base/multiplier split (×{cfg['multiplier_floor']:g}–×{cfg.get('multiplier_ceiling') or float('inf'):g}) is a diagnostic normalization only. <a href='FILL.md'>Starting fill evaluation</a></p>"
...
<style>.map-tabs{display:flex;gap:8px;margin:22px 0 8px;flex-wrap:wrap}.map-tabs button[aria-pressed="true"]{background:#315e7e;border-color:#87c4ed;color:white}.map-tabs button:focus-visible{outline:2px solid #a6d8f7;outline-offset:3px}#viewNote{font-size:13px;margin:8px 0}</style><main><header><div><h1>EU5 location capacity</h1><p>People per location · natural capacity, inherited improvements and feasible maximum</p></div><div class="tag">Iteration 01 · all map zones covered<br>Historical calibration remains provisional</div></header>
<div class="stats"><div><strong>__COUNT__ / __COUNT__</strong><span>map zones with complete targets</span></div><div><strong id="startingTotal">__START__</strong><span>starting support</span></div><div><strong id="maximumTotal">__MAX__</strong><span>maximum scenario support</span></div></div>
...
<p id="areaNote">Equal-area model: every land location uses the same __REFERENCE_AREA__ ha reference. Location size does not scale capacity. No unit floor and no population-based allowance enter the targets.</p><div class="layout"><section><div class="map"><canvas id="map"></canvas></div><div class="legend"><span>0</span><div class="bar"></div><span id="scale"></span></div><small>20,929 modelled land locations; 7,644 nonsettlement zones have explicit zero capacity. Drag to pan · scroll to zoom · click a location. Colour saturation is a display limit; values are not capped.</small></section><aside id="detail"><h2>Select a location</h2><p>Each location has all four required estimates. Search includes small islands that may disappear at world-view resolution.</p><p>Maximum improvements include existing improvements.</p></aside></div>
<p>Capacity = natural capacity + typed improvements, all in people. The maximum holds the crop system fixed and allows additional clearing and constrained surface-water investment. Physical land and water are accounted separately.</p>
<div class="links"><a id="valuesLink" href="capacity_targets_equal_area.csv">Capacity targets</a><a href="improvement_ledger_equal_area.csv">Typed improvement ledger</a><a id="ledgerLink" href="locations_equal_area.csv">Full location ledger</a><a href="REPORT.md">Iteration findings</a><a href="IMPROVEMENTS.md">Improvement audit</a><a href="WATER_MANAGEMENT.md">Water-management evidence</a><a href="water_management_ledger.csv">Water ledger</a><a href="improvement_distribution_equal_area.csv">Improvement shares</a><a href="rural_pressure.html">Rural pressure</a> · <a href="regional_comparison.html">Regional changes</a><a href="RURAL_BALANCE.md">Rural game allowance</a><a href="validation.json">Validation</a><a href="manifest.json">Source and parameter manifest</a><a href="location_ids_native.png">Native location geometry</a></div>
<footer>Fingerprint __HASH__ · Modern environmental proxies, historical evidence around 1300. Historical estimates are population-independent. Water-management and land-access estimates carry uncertainty.</footer></main>
...
function show(i){
  ...
  document.getElementById('detail').innerHTML=
    ...
    '<h3 class="input-heading">Capacity targets (people)</h3>'+
    input('Natural capacity',whole(d.inert_capacity),(d.rural_balance_added_capacity>0?'Includes '+whole(d.rural_balance_added_capacity)+' capacity of explicit rural game allowance':'Land before represented improvements'))+
    input('Existing improvements',whole(d.starting_improvement_capacity),'Already present at game start')+
    input('Maximum improvements',whole(d.maximum_capacity-d.inert_capacity),'Total limit, including existing improvements')+
    '<p class="model-formula">Capacity = natural + improvements</p><p class="unit-note">All values are people in equal-area game units. The base/multiplier normalization is a diagnostic in the breakdown. Display values are rounded.</p>'+
