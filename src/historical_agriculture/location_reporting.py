...
const tabSelections={};
function metricTab(m){return /^(starting|maximum)_(clearing|management|irrigation)_improvement_capacity$/.test(m.key)?'improvements':m.key==='starting_fill'?'pressure':'capacity'}
function setTab(tab){
...
function show(i){
  ...
  for(const [kind,label] of [['clearing','Clearing'],['management','Field management'],['irrigation','Irrigation']]){
